...
let qaoa_costtime = [];
let dijk_costTime = [];
let dijk_executionTimes = []; // Store execution time for each location
let qaoa_executionTimes = [];
function initializeMap(position) {
    ...
    function optimizeRoute() {
        ...
        while (remainingLocations.length > 0) {
            ...
            for (let location of remainingLocations) {
            ...
            let executionTime = (endTime - startTime).toFixed(7); // Convert to 4 decimal places for precision
            Totalexecution=Totalexecution+(executionTime)
            dijk_executionTimes.push(parseFloat(Totalexecution)); // Store execution time in ms
        ...
        console.log("Dijkstra Execution Times per location (ms):", dijk_executionTimes);

        (async function () {

            waypoints,qaoa_executionTimes = await quantumoptimizeRoute();

            let data_qaoa = waypoints;
            qaoa_costtime = await Calculate_Cost_Time(); //  Await is valid here
            console.log(qaoa_executionTimes)
    ...
    async function quantumoptimizeRoute() {
        try {
            ...
            // Validate waypoints before sending request
            if (!Array.isArray(waypoints) || waypoints.length < 2 || waypoints.some(coord => isNaN(coord[0]) || isNaN(coord[1]))) {
            ...
            // Assign new values
            let optimizedRoute = data.optimized_route;
            let executionTimes = data.execution_time; // Store execution time list
    
            console.log("Optimized Route:", optimizedRoute);
            console.log("Execution Times (ms):", executionTimes);
    
            // Display execution time details
            let executionSummary = executionTimes.map((time, index) => `Step ${index + 1}: ${time} ms`).join("\n");
            alert("Optimized Route: " + JSON.stringify(optimizedRoute) + "\nExecution Times:\n" + executionSummary);
    
            return optimizedRoute,executionTimes;
        } catch (error) {
    ...
    // function showExecutionTimeGraph() {
    //     let ctx = document.getElementById('executionTimeChart').getContext('2d');
    //     let noDataMessage = document.getElementById('noDataMessage');
    //     let chartCanvas = document.getElementById('executionTimeChart');
    
    //     // Check if data exists before rendering the chart
    //     if (dijk_executionTimes.length && !qaoa_executionTimes.length) {
    ...
    //         type: 'line',
    //         data: {
    //             labels: dijk_executionTimes.map(data => data.nodes), // X-axis: Number of Nodes
    //             datasets: [
    //                 {
    //                     label: "Dijkstra's Algorithm",
    //                     data: dijk_executionTimes.map(data => data.time), // Y-axis: Execution Time
    ...
    //                 {
    //                     label: "QAOA",
    //                     data: qaoa_executionTimes.map(data => data.time), // Y-axis: Execution Time
    ...
    //                             let nodes = dijk_executionTimes[index]?.nodes ?? qaoa_executionTimes[index]?.nodes ?? "Unknown";
    //                             let timeDijk = dijk_executionTimes[index]?.time ?? "N/A";
    //                             let timeQaoa = qaoa_executionTimes[index]?.time ?? "N/A";
    //                             return [
    //                                 `Nodes: ${nodes}`,
    //                                 `Dijkstra's Time: ${timeDijk} ms`,
    //                                 `QAOA Time: ${timeQaoa} ms`
    //                             ];
    //                         }
    //                     }
    //                 },
    ...
    function showCostGraph() {
        ...
        let algorithm_executiontime = `           QAOA: ${(qaoa_executionTimes[qaoa_executionTimes.length - 1]).toFixed(3)} ms             Dijkstra's: ${dijk_executionTimes[dijk_executionTimes.length - 1]} ms`;
        ...
        // document.getElementById('Executiontime').innerText = algorithm_executiontime
        if (costChart) {
        ...
        costChart = new Chart(ctx, {
                ...
                datasets: [
                    {
                        ...
                        borderColor: 'rgb(84, 93, 219)',
                        backgroundColor: 'rgba(59, 32, 211, 0.84)',
                        borderWidth: 2
                    },
                    {
                        label: "Dijkstra's",
                        data: qaoa_costtime.map(data => data.time),
                        borderColor: 'rgb(211, 79, 79)',
                        backgroundColor: 'rgb(206, 26, 26)',
                        borderWidth: 2
                    }
                ]
            },
            options: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (tooltipItem) {
                                ...
                                return [
                                    `Cost: ₹${cost}`,
                                    `QAOA Time: ${timeDijk} ms`,
                                    `Dijkstra's Time: ${timeQaoa} ms`
                                ];
                            }
                        }
                    },
                ...
                scales: {
                    ...
                    y: { title: { display: true, text: ' Execution Time (milisecond)' }, beginAtZero: true }
                }
            },
